...
import com.gruppomcr.dao.interfacce.IImpiegati;
import com.gruppomcr.dao.model.ImpiegatiDAOModel;
import com.gruppomcr.persistence.ConnectionManager;

public class ImpiegatiDAO implements IImpiegati {
	ImpiegatiDAOModel userbean;
	...
	public void eseguiDelete(int id) throws Exception {
		// TODO Auto-generated method stub
		        ...
		        try {
		            ...
		            java.sql.PreparedStatement query = conn.prepareStatement("UPDATE impiegati SET isValid=0 WHERE ID=?");
		            query.setInt(1, id);
	...
	public void eseguiInsert( String cognome, int id_dipartimento,boolean isValid) throws Exception {
        ...
        try {
            ...
            // crea un istanza della classe PreparedStatament che � una sottoclasse di Statament per eseguire istruzioni sql
            java.sql.PreparedStatement query = conn.prepareStatement("INSERT INTO impiegati VALUES ((SELECT MAX(id)+1 FROM impiegati as imp),?,?,?)");
            
            query.setString(1, cognome);
            query.setInt(2, id_dipartimento);
            query.setBoolean(3, isValid);
	...
	public ImpiegatiDAOModel eseguiSelectById(int id) throws Exception {
        ...
        try {
            ...
            // crea un istanza della classe PreparedStatament che � una sottoclasse di Statament per eseguire istruzioni sql
            java.sql.PreparedStatement query = conn.prepareStatement("select * from impiegati where id = ?");
            query.setInt(1, id);
            ...
            if (ris.next()) {
            	...
            	userbean = new ImpiegatiDAOModel();
                userbean.setId(ris.getInt("id"));
                userbean.setCognome(ris.getString("nome"));
                userbean.setId_dipartimento(ris.getInt("id_dipartimento"));
            }
	...
	public void eseguiUpdateById(int id, String cognome, int id_dipartimento) throws Exception {
        ...
        try {
            ...
            // crea un istanza della classe PreparedStatament che � una sottoclasse di Statament per eseguire istruzioni sql
            java.sql.PreparedStatement query = conn.prepareStatement("UPDATE impiegati SET  cognome=? id_dipartimento=? isValid=? WHERE id = ?");
            
            query.setString(1, cognome);
            query.setInt(2, id_dipartimento);
            if(id_dipartimento!=0) {
            	query.setInt(3, 1);
            }else {
            	query.setInt(3, 0);
            }
	...
	public List<ImpiegatiDAOModel> eseguiSelect() throws Exception {
		ConnectionManager connM = new ConnectionManager();
	     Connection conn = null;
	     ResultSet ris;
	     List<ImpiegatiDAOModel> lista = new ArrayList<ImpiegatiDAOModel>();
	     try {
